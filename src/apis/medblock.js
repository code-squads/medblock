...
export async function universalLogin(pk, authorityType){
    return new Promise((resolve, reject)=>{
        if(!authorityType)
            return reject("Authority type required for login !!!");

        if(!isValidPrivateKey(pk))
          return reject("Invalid private key:" + pk);
        ...
        switch(authorityType){
            ...
            case AUTHORITY_TYPES.HOSPITAL:
                getHospitalInfo(newWallet.address)
                    .then(resolve)
                    .catch(reject);
                break;
            case AUTHORITY_TYPES.PATIENT:
                getPatientPersonalInfo(newWallet.address)
                    .then(resolve)
                    .catch(reject);
                break;
            default:
...
export async function getRecordHistory(patientAddress){
    return new Promise((resolve, reject) => {
        if(!patientAddress)
            return reject(new Error("Invalid patient address"));

        MedBlock.methods
            .getAllRecords(patientAddress).call()
            .then(res => {
                console.log(`Record history for ${patientAddress}: \n`, res);
                resolve(res);
            }).catch(err => {
                console.log("Some error obtaining record history! \n", err);
                reject(new Error(`Couldn't fetch record history for patientAddress: ${patientAddress}`));
            })
    });
}
...
export async function getRecord(patientAddress, recordID){
    return new Promise((resolve, reject) => {
        ...
        MedBlock.methods
            .getRecord(patientAddress, recordIDNumeric).call()
            .then(record => {
                const recordObj = {...record, recordID: recordIDNumeric};
                console.log("Record details:\n", recordObj);
                resolve(recordObj);
            }).catch(err => {
                console.log(`Some error fetching record no. ${recordIDNumeric} \n`, err);
...
export async function addNewRecord(
    ...
    const dischargeDateString = wasAdmitted ? dateToTimestamp(dischargeDateObj) : 0;
    
    const confirmation = window.confirm("Do you really want to add this record ? \nThis can't be undone...");
    if(!confirmation)
        return;
...
export async function approveRecord(accountAddress, recordDetails){
    ...
    const confirmation = window.confirm("Do you really want to approve this record ? \nThis can't be undone...");
    if(!confirmation)
        return;
    ...
    const recordIDNumeric = Number(recordID);
    console.log(`Approving record no. ${recordIDNumeric} of account:`, accountAddress);
...
export async function declineRecord(accountAddress, recordDetails, declineMsg){
    ...
    const confirmation = window.confirm("Do you really want to decline this record ? \nThis can't be undone...");
    if(!confirmation)
        return;
    ...
    const recordIDNumeric = Number(recordID);
    console.log(`Declining record no. ${recordIDNumeric} of account:`, accountAddress);
...
export async function newHospitalRegistration(
    ...
    const confirmation = window.confirm("Do you really want to register hospital: \n" + name);
    if(!confirmation)
        return;
...
export async function newPatientRegistration(
    ...
    if(!fname || !lname || !birthdate || !phone || !residentialAddress || !patientBlockchainAddress || !figureOutGender(gender))
        return console.log("Required parameters not provided!");

    const confirmation = window.confirm("Do you really want to register patient: \n" + fname + " " + lname);
    if(!confirmation)
        return;
...
export function isHospitalAlreadRegistered(hospitalBlockchainAddress){
    return new Promise((resolve, reject) => { 
        MedBlock.methods
            .getHospitalInfo(hospitalBlockchainAddress).call()
            .then(res => {
...
export function isPatientAlreadyRegistered(patientBlockchainAddress){
    return new Promise((resolve, reject) => { 
        MedBlock.methods
            .getUserInfo(patientBlockchainAddress).call()
            .then(res => {
